...
from rmlmodels.LSTMModel import LSTMNet, CNNLSTM_Net, LSTMNet11
...
def get_upper_SNR_only_AMSSR(lbl,snrs,data_idx,X_data,Y_data):
    mask_list = []
    data_lbl = [lbl[x][0] for x in data_idx]
    for snr in snrs:
        if snr >= -6:
            data_SNRs = [lbl[x][1] for x in data_idx]
            mask_list.append(np.where(np.array(data_SNRs) == snr))
    mask = np.intersect1d(np.concatenate(mask_list),np.array(data_lbl) == 'AM-SSB')
...
train_idx, val_idx, test_idx) = rmldataset2016_11.load_data()

# 获取比-6dB大的数据集，训练集只使用AM-SSB的数据
X_train, Y_train = get_upper_SNR_only_AMSSR(lbl, snrs, train_idx, X_train, Y_train)
...
model = LSTMNet11().to(device)
# model = CNNLSTM_Net().to(device)
# model = Net().to(device)

Frozen_model = LSTMNet().to(device)
...
# 加载并冻结模型
Frozen_model.load_state_dict(torch.load(weight_path + weight_file_name))
...
loss_func = nn.CrossEntropyLoss()  # 分类问题 # alpha*loss_func(torch.cat((prediction[:,:2],prediction[:,3:]),dim=1),Frozen_prediction)
KL_loss_func = nn.KLDivLoss(reduction='batchmean')  # alpha*KL_loss_func(torch.log(torch.cat((prediction[:,:2],prediction[:,3:]),dim=1)),Frozen_prediction)
cos_loss_func =  nn.CosineEmbeddingLoss()# + alpha*cos_loss_func(torch.cat((prediction[:,:2],prediction[:,3:]),dim=1),Frozen_prediction,torch.ones(Frozen_prediction.size(0)).to(device))
...
for i in range(EPOCH):
    ...
    for step, (batch_data, batch_label) in enumerate(train_loader):
        ...
        Frozen_prediction = Frozen_model(batch_data)
        
        loss = (1-alpha)*loss_func(prediction, batch_label) + alpha*cos_loss_func(torch.cat((prediction[:,:2],prediction[:,3:]),dim=1),Frozen_prediction,torch.ones(Frozen_prediction.size(0)).to(device))
    ...
    with torch.no_grad():
        for step, (batch_data, batch_label) in enumerate(valid_loader):
            ...
            loss = (1-alpha)*loss_func(prediction, batch_label) + alpha*KL_loss_func(torch.log(torch.cat((prediction[:,:2],prediction[:,3:]),dim=1)),Frozen_prediction)
    ...
    acc_list.append(acc)
    if acc > max_acc:
        print(f"This is max_acc:{acc}")
        model_weight = model.state_dict()
    max_acc = max(acc, max_acc)

print(acc_list)
print(f"save model in {weight_path}")
torch.save(model_weight, weight_path + f'incremental:{alpha}_params-{max_acc}.pth')