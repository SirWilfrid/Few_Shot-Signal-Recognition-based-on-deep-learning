...
from rmlmodels.LSTMModel import LSTMNet11
...
def predict(model,filepath):
    ...
    mltools.plot_confusion_matrix(confnorm, labels=['8PSK','AM-DSB','AM-SSB','BPSK','CPFSK','GFSK','4-PAM','16-QAM','64-QAM','QPSK','WBFM'],save_filename='figure11/lstm3_total_confusion.png')
    ...
    for snr in snrs:
        ...
        # estimate classes
        with torch.no_grad():
            test_Y_i_hat = model(torch.tensor(test_X_i).to(device))
        confnorm_i,cor,ncor = mltools.calculate_confusion_matrix(test_Y_i,test_Y_i_hat.cpu().numpy(),classes)
        acc[snr] = 1.0 * cor / (cor + ncor)
        result = cor / (cor + ncor)
        with open('acc111.csv', 'a', newline='') as f0:
            write0 = csv.writer(f0)
            write0.writerow([result])
        mltools.plot_confusion_matrix(confnorm_i, labels=['8PSK','AM-DSB','AM-SSB','BPSK','CPFSK','GFSK','4-PAM','16-QAM','64-QAM','QPSK','WBFM'], title="Confusion Matrix",save_filename="figure11/Confusion(SNR=%d)(ACC=%2f).png" % (snr,100.0*acc[snr]))
    ...
    for g in range(int(np.ceil(acc_mod_snr.shape[0]/dis_num))):
        ...
        plt.legend()
        plt.grid()
        plt.savefig('figure11/acc_with_mod_{}.png'.format(g+1))
        plt.close()
    ...
    plt.tight_layout()
    plt.savefig('figure11/each_acc.png')


if __name__ == '__main__':
    filepath = "/root/LSTM2torch/weights/incremental:0.2_params-0.2611888111888112.pth"
